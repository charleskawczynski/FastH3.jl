...
H3X.jl is a pure Julia reimplementation with zero binary or C dependencies. This matters for several reasons:

- **No binary dependencies** -- H3.jl depends on `BinaryProvider` to distribute pre-compiled `libh3` binaries. H3X.jl has no compiled C code, no JLL packages, and no platform-specific binaries. This eliminates build failures on unsupported or exotic platforms and simplifies installation.
...
- **No heap-allocating `Ref` wrappers** -- The H3 C API writes results through output pointers, so H3.jl must allocate a mutable `Ref{T}()` (a heap-allocated `Base.RefValue{T}`) for every call that returns a scalar -- `Ref{H3Index}()`, `Ref{Int64}()`, `Ref{Cdouble}()`, `Ref{LatLng}()`, `Ref{CellBoundary}()`, etc. H3X.jl returns results directly as tuples, avoiding those per-call heap allocations entirely.
